    ...
    try {
      switch (type) {
        ...
        case 'set_as_speaker':
          ...
          const {position, role} = await roomAction(token, roomId, type, {permitToken, targetId});
          payload = { id: targetId, position : String(position),  role};
          console.log(payload)
          await sendToRoom('success', roomId, payload, [], type);
          break;
        ...
        case 'set_as_viewer':
          const viewerResult = await roomAction(token, roomId,type, {targetId});
        ...
        case 'remove_from_host':
          const removeResult = await roomAction(token, roomId, type, {targetId});
        ...
        case 'set_as_host':
          const hostResult = await roomAction(token, roomId, type, {targetId});
          if (hostResult.error) {
        ...
        case 'remove_from_room':
          roomActionResult = await roomAction(token, roomId,  type, {targetId})
          if(roomActionResult.error){
            sendToUser('error', userId, roomActionResult.error, type);
            return;
          }
          payload = {id: targetId};
          await sendToRoom('success', roomId, payload, [], type);
          sendToUser('success', targetId, "removed", type);
          break;

        // Disable sending of messages
        case 'pause_messaging':
          roomActionResult = await roomAction(token, roomId, type, {});
          if(roomActionResult.error){
            sendToUser('error', userId, roomActionResult.error, type);
            return;
          }
